...
u8 IIC_Wait_Ack(void)
{
	u8 ucErrTime=0;
	SDA_IN();      
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5,GPIO_PIN_SET);delay_us(1);	   
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_4,GPIO_PIN_SET);delay_us(1);	 
...
u8 IIC_Read_Byte(unsigned char ack)
{
    ...
    for(i=0;i<8;i++ )
	{
		...
		HAL_GPIO_WritePin(GPIOB,GPIO_PIN_4,GPIO_PIN_SET);
        receive<<=1;
        if(HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_5))receive++;   
		delay_us(1); 
    }					 
    if (!ack)
        IIC_NAck();
